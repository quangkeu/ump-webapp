$(function () {
    ...
    function firstLoad() {
        ...
        treeTagsHtml.fancytree({
            checkbox: false,
            icon: false,
            source: [],
            activate: function (event, data) {
                currentTagId = data.node.key;

                // Update list parameters
                updateListParentObjects();
                ump.updateUrlQueryString('object', null);
                ump.updateUrlQueryString('tag', currentTagId);
    ...
    btnDeleteObjectHtml.click(function () {
        if (confirm('Do you want to delete ' + currentParentObjectPath + '?')) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    location.replace(location.origin + location.pathname + '?tag=' + ump.getUrlQueryValue('tag'));
                }
            })
        }
    });
    ...
    function generateTreeParentObject(input, resultSearch) {
        ...
        treeObjectHtml.find('.fancytree-title').each(function () {
            var currentParentObject = $(this).text();
            $(this).attr('title', currentParentObject);
        });
    }

    function generateTreeTag(input) {
        var active = currentTagId && currentTagId != '' ? currentTagId : '';
        var output = [];
        var profileSearchSuccess = [];
        $.each(input, function (index, value) {
            if (value.resultSearch) {
                profileSearchSuccess.push(value.name);
            }
        });
        $.each(input, function (index, value) {
            var node = {key: value.id, title: value.name};
            if(value.id == active) {
                node.active = true;
            }
            if (profileSearchSuccess.indexOf(value.name) >= 0) {
                node.selected = true;
            }
            output.push(node)
        });

        treeTagsHtml.fancytree("getTree").reload(output).done();



        treeTagsHtml.find('.fancytree-title').each(function () {
            var currentProfileName = $(this).text();
            $(this).attr('title', currentProfileName);
            // if (profileSearchSuccess.indexOf(currentProfileName) >= 0) {
            //     $(this).css('background-color', '#F6F792');
            // }
        });
    }
    ...
    function updateListParameters() {
        ...
        $.ajax({
            type: "GET",
            url: document.location.origin + '/devices/device_settings/' + currentDeviceId + '/get-list-parameters',
            data: {
                parentObject: currentParentObjectPath,
                tagId: currentTagId
            },
    ...
    function updateListTags() {
        ...
        $.ajax({
            type: "GET",
            url: document.location.origin + '/devices/device_settings/' + currentDeviceId + '/get-list-tags',
            data: {
            ...
            success: function (response) {
                ...
                var _listTags = response.listTags && response.listTags.length > 0 ? response.listTags : [];

                currentTagId = ump.getUrlQueryValue('tag');
                currentTagId = currentTagId != null && currentTagId != '' || _listTags.length === 0 ? currentTagId : _listTags[0].id;

                updateListParentObjects();
                generateTreeTag(_listTags);
                ump.updateUrlQueryString('tag', currentTagId);
            }
        })
    }
    ...
    function updateListParentObjects() {
        ...
        $.ajax({
            type: "GET",
            url: document.location.origin + '/devices/device_settings/' + currentDeviceId + '/get-list-parent-objects',
            data: {
                parentObject: currentPathSearch,
                tagId: currentTagId
            },
            success: function (response) {
                ...
                currentParentObjectPath = currentParentObjectPath !== null && currentParentObjectPath !== '' || _listParentObjects.length === 0 ? currentParentObjectPath : _listParentObjects[0];

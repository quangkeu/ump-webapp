$(function () {
    ...
    policy_configuration_form.find('input[name="radioLimited"]').change(function () {
        if($(this).val() === '0') {
            policy_configuration_form.find('input[name="limited"]').hide().val('');
        } else {
            policy_configuration_form.find('input[name="limited"]').show();
        }
    });
    ...
    function initRemoveParameter() {
        policy_configuration_form.find('.remove-parameter-btn').unbind( "click" )
            .click(function () {
                ...
                if(parametersByPath[pathRemove]) {
    ...
    function generateParameters() {
        $.ajax({
            type: 'GET',
            url: location.origin + '/policy/' + $('.policy-configuration-form').attr('data-policyId'),
            success: function (response) {
                $.each(response.parameters, function (index, value) {
                    addParameter(value)
                });
            }
        });
